...
run_compare() {
    ...
    MSI_THRESHOLD=8000
    mkdir -p $DIFF_DIR/mse
    mkdir -p $DIFF_DIR/ssim
...
mse_compare() {
    ...
    local diff_image="$DIFF_DIR/mse/$basename"
    local mse_diff=$(magick compare -metric MSE "$image_1" "$image_2" "$diff_image" 2>&1)
    if [ $? -eq 0 ]; then
        echo "$mse_diff"
    else
        echo "10000 (1)"
...
ssim_compare() {
    ...
    local image_2=$2
    local basename=$(basename "$image_1")
    local diff_image="$DIFF_DIR/ssim/$basename"
    local ssim_diff=$(magick compare -metric SSIM "$image_1" "$image_2" "$diff_image" 2>&1)
    if [ $? -eq 0 ]; then
        echo "$ssim_diff"
    else
        echo ""
